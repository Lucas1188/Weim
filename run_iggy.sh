...
echo
echo "timeout \"$MAX_DURATION\" afl-fuzz -n -i \"$SEEDS_DIR\" -o \"$OUT_DIR\" -t \"$TIMEOUT_PER_INPUT+\" -- ./iggy.py @@"
echo
echo "[*] After fuzzing, package results with:"
echo ./pack_logs.sh
echo "./hound.py" > pack_logs.sh 
echo "tar -czf \"${HARNESS_ROOT_DIR}/$(basename "$TARGET_BIN").tar.gz\" \"$OUT_DIR\" \"$HARNESSLOG_DIR\" \"$SEEDS_DIR\" \"$FINAL_BUGS\" " >> pack_logs.sh
